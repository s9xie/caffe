 ...
 */
template <typename Dtype>
class ImageContextDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit ImageContextDataLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~ImageContextDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_IMAGE_CONTEXT_DATA;
  }
  virtual inline int ExactNumBottomBlobs() const { return 0; }

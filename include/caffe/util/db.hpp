...
class LMDBCursor : public Cursor {
 public:
  explicit LMDBCursor(MDB_txn* mdb_txn, MDB_cursor* mdb_cursor)
    : mdb_txn_(mdb_txn), mdb_cursor_(mdb_cursor), valid_(true) {
    SeekToFirst();
  }
  virtual ~LMDBCursor() { mdb_cursor_close(mdb_cursor_); }
  virtual void SeekToFirst() {
    MDB_CHECK(mdb_cursor_get(mdb_cursor_, &mdb_key_, &mdb_value_, MDB_FIRST));
    valid_ = true;
  }
  virtual void Next() {
